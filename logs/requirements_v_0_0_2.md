...
#### 生成数字身份
用户点击“生成数字身份”按钮，App生成一对新的私钥和地址，此地址在ERC1056默认注册为identity，用户在升级账户时此地址可在ERC1484注册为关联地址获得EIN，私钥需要提醒用户使用冷钱包保存
#### 找回数字身份
此操作只有升级过的数字身份可以使用。
用户手动输入加密过的恢复密钥，然后解密用来恢复关联地址丢失或被盗用的数字身份，此操作会重置所有关联地址，然后由用户手动选择新地址亦或者重新选择未丢失或未被盗用的关联地址添加至链上成为新的关联地址以重新取回EIN的唯一管理权
#### 导入数字身份
用户点击“导入数字身份”按钮，输入提前创建好或已拥有的私钥，App先查询此地址在ERC1484是否已有关联好的EIN，若有到ERC1484读取获得EIN；若没有App将私钥对应的地址在ERC1056注册为identity，私钥需要提醒用户使用冷钱包保存
#### 地址管理
用户可以像传统钱包一样管理自由管理任意数量地址，每个地址都具有传统的数字货币属性，但需要对特殊的关联地址或ST认证过的地址等进行标记。

### 数字身份升级
#### 账户升级
用户账户可从单纯的ERC1056 DID升级为拥有恢复功能的ERC1484 EIN账户，原来ERC1056 identity对应的地址被设为关联地址，identity的Owner设为ERC1056Resolver的合约地址；另外，需要为用户生成一个恢复密钥并严格提醒用户冷钱包存储且App不存储此密钥。
#### 关联地址管理
用户可以选择生成任意数量的地址并添加为关联地址，每个地址都具有传统的数字货币属性或ST属性
#### 恢复地址管理
